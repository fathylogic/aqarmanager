...
<div class="row g-3">
                          <div class="col-md-6">
                            ...
                            <input type="text" id="center_name" name="center_name" value="{{ $center->center_name }}" class="form-control" required   />
                          </div>
                          <div class="col-md-6">
                            ...
                            <select id="center_location" name="center_location" required class="select2 form-select" data-allow-clear="true">
                              <option value="">اختر</option>
                               @foreach ($locations as $row)
                                 <option value="{{ $row->id }}" @if($row->id == $center->center_location) {{ 'selected' }}@endif>{{ $row->name }}</option>
                               @endforeach
                          ...
                          <div class="col-md-6">
                            ...
                            <input type="text" id="electric_no" value="{{ $center->electric_no }}" name="electric_no"  class="form-control"   />
                          </div>
 
                          <div class="col-md-6">
                            <label class="form-label" for="woter_no">  حساب شركة المياة   <i class="fa fa-asterisk " style="color: red" aria-hidden="true"></i></label>
                            <input type="text" id="woter_no" name="woter_no" value="{{ $center->woter_no }}"  class="form-control"   />
                          </div>

                           <div class="col-md-6">
                            <label class="form-label" for="left_electric_no">  حساب   اخر للمصاعد    </label>
                            <input type="text" id="left_electric_no" value="{{ $center->left_electric_no }}" name="left_electric_no" class="form-control"   />
                          </div>
                             <div class="col-md-6">
                          <label for="file" class="form-label">   صورة

<?php $url = asset('storage/'.$center->img) ; ?> <a href="{{ $url }}" target="_blank" >  <i class="fa fa-eye "  aria-hidden="true"></i>   </a>
